library(BioCro)
#check out to this branch: add_new_water_stress_function
library(BMLePhoto)
rm(list=ls())
...
weather <- soybean_weather[names(soybean_weather)==year][[1]] 

#------------------------
#Currently, NO need to change "restart" since it's automatically addressed below!!
#by comparing the start_day and sowdate
#
...
out_filename_prefix  = paste0(output_folder,"/results_")
if(restart){
  #if it's restart, then we start with the DOY one day before the start_day
  last_filename = paste0(output_folder,"/results_",start_day-1,".rds")
}
...
# soybean_parameters
soybean_parameters = soybean2$parameters
soybean_parameters = set_ld11_parameters(year,soybean_parameters)
...
#get the last day's record to use as initial
   ...
   # print(as.data.frame(soybean_initial_state_last))
